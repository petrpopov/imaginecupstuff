...
namespace Anganar
{
	class MyAI : VG.Common.Player 
	{
        ...
        //Если и такой нет, то CurLocation
        public Point GetNextHPForConvoy(Point CurLocation, ref int Number)
        {
            Point tmp = new Point(-1, -1);
            int ntmp;
            for (int i = 0; i < MyHPs.Count; i++)
        ...
        public Point GetNextUndoneNavPoint(Point CurLocation, ref int Number)
        {
            ...
            for (int i = 0; i < NavigationPoints.Length; i++)
            {
                ...
                if ((tmp < MinNavs) && (NavigationPoints[i].Complete == false) && ((NavigationPoints[i].BotType == NanoBotType.Unknown) || (NavigationPoints[i].BotType == NanoBotType.NanoExplorer)))
                {
        ...
        public Point GetNextUndoneHealPoint(Point CurLocation, ref int Number)
        {
            ...
            {
                tmp = NavigationPoints[i].Navigators;
                if ((tmp < MinNavs) && (NavigationPoints[i].Complete == false) && (NavigationPoints[i].BotType == NanoBotType.NanoCollector))
                {
                    MinNavs = NavigationPoints[i].Navigators;
                    Number = i;
                }
            ...
            for (int i = 0; i < mission.Count; i++)
            {
                ...
                else if (mission[i].ID == 1)
                {
                    ...
                    for (int j = 0; j < navObj.NavPoints.Count; j++)
                    {
                        ...
                        NavigationPoints[j].Navigators = 0;
                        NavigationPoints[j].Complete = false;
                        NavigationPoints[j].BotType = navObj.NanoBotType;
                        NavigationPoints[j].Stock = navObj.NavPoints[j].Stock;
                    }
                }
        ...
        void MyAI_WhatToDoNextEvent()
		{
			...
			if (this.AI.State == NanoBotState.WaitingOrders)
			{
                ...
                else
                {
                    ...
                    if (!STOPPED)
                    {
                        //Проверяем, все ли ещё живы телохранители готовы двигаться
                        for (int i = 0; i < BodyGuards.Length; i++)
                        {
		...
		void MyAI_ChooseInjectionPointEvent()
		{
            ...
            //{
                ...
                if (NavigationPoints != null)
                {
                    for (int i = 0; i < NavigationPoints.Length; i++)
                    {
                        if (NavigationPoints[i].BotType == NanoBotType.NanoCollector)
                        {
                            NBDOCTORSTOBUILD++;
                        }
                        else if ((NavigationPoints[i].BotType == NanoBotType.NanoExplorer) || (NavigationPoints[i].BotType == NanoBotType.Unknown))
                        {
                            NBNAVIGATORTOBUILD++;
                        }
                    }
            ...
            int NotAllocatedBots = Utils.NbrMaxBots - 4 - NBATACKERTOBUILD - NBNAVIGATORTOBUILD - NBDOCTORSTOBUILD - NBPROTECTORTOBUILD - MyHPs.Count;
            int ConvoysNumber = Math.Min((NotAllocatedBots - (NotAllocatedBots % 3)) / 3, MyHPs.Count);
