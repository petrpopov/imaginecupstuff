...
namespace MIPT
{
	class MyAI : VG.Common.Player 
	{
        ...
        private ForwardBodyguard[] FBGs = null;
        ...
        //Массив всех UniqueNavigationPoints
        public NavigationPointInfo[] UniqueNavPoints = null;
        ...
        public int NBATACKERTOBUILD = 0;   //Атакует цели, указанные в списке ATargets
        ...
        public Point GetNextUngettedHP(Point CurLocation, ref int Num)
        {
            Point tmp = new Point(-1, -1);
            int Number;
            //Место вызова перестроения & Замена неподходящих Num`ов???
            //Если не первый вызов
            if (Num >= 0)
            {
                //Если текущая точка занята противником
                if (HoshimiPoints[Num].Needle == 2)
                {
                    //Ищем новую (ближайшую)
                    int dist = 10000;
                    for (int i = 0; i < HoshimiPoints.Length; i++)
                    {
                        if (HoshimiPoints[i].Needle == 0)
                        {
                            int aaa = 0;
                            ePathfinder.FindPath(this.AI.Location, HoshimiPoints[i].Location, ref aaa);
                            aaa = aaa * (StepCost(this.AI.Location) + StepCost(HoshimiPoints[i].Location)) / 2;
                            if (aaa < dist)
                            {
                                dist = aaa;
                                Num = i;
        ...
        public Point GetNextHPForConvoy(Point CurLocation, ref int Number, ref bool Navigating)
        {
            ...
            for (int i = 0; i < HoshimiPoints.Length; i++)
            {
                if ((HoshimiPoints[i].Convoys == 0) && (HoshimiPoints[i].Full == 0) && (HoshimiPoints[i].Needle == 1))
                {
                    Number = i;
                    HoshimiPoints[i].Convoys++;
                    return HoshimiPoints[i].Location;
                }
            }
            for (int i = 0; i < MyHPs.Count; i++)
            {
        ...
        public Point GetNextCyclicNavPoint(NavigationPointInfo[] UNPs, ref int Number)
        {
            Number = -10;
            for (int i = 0; i < UNPs.Length; i++)
            {
                if (UNPs[i].Complete == false)
                {
                    Number = i;
                    return UNPs[i].Location;
                }
            }
            NBCYCLICNAV = 0;
            NBCYCLICDOC = 0;
            return UNPs[UNPs.Length-1].Location;
        }
        ...
        //Направляет телохранителей AI по заданному пути
        private void DirectFBGs()
        {
            for (int i = 0; i < FBGs.Length; i++)
            {
                if (FBGs[i] != null && FBGs[i].HitPoint > 0)
                {
                    Point[] path = this.Pathfinder.FindPath(FBGs[i].Location, NextHP);
                    FBGs[i].MoveTo(path);
                }
            }
        }
        ...
        //"Регистрирует" нового телохранителя в команде телохранителей AI
        void RegisterFBG(ForwardBodyguard bg)
        {
            int tmp = -1;
            for (int i = 0; i < FBGs.Length; i++)
            {
                if (FBGs[i] == null && tmp == -1)
                    tmp = i;
            }
            if (tmp != -1)
                FBGs[tmp] = bg;
        ...
        private bool CheckBlockers(bool STOPPED)
        {
            ...
            if (OtherInjectionPointsInfo != null)
            {
                ...
                {
                    if (ipi.PlayerID != 0 && GeomDist(AI.Location, ipi.Location) <= Utils.BlockerStrength)
                    {
                        ip = true;
                        target = ipi.Location;
                    }
            ...
            foreach (NanoBot bot in NanoBots)
            {
                if ((((bot is NanoNeedle) || (bot is NanoBlocker)) || (bot is NanoNeuroControler)) && (bot.Location == AI.Location))
                    possible = false;
                if ((bot is NanoBlocker) && (GeomDist(bot.Location, target) <= Utils.BlockerStrength - 1))
                    return false;
            }
            if (OtherNanoBotsInfo != null)
            {
        ...
        {
            Point ret = new Point();
            ret.X = (int)(start.X + d * (end.X - start.X) / GeomDist(start, end));
            ret.Y = (int)(start.Y + d * (end.Y - start.Y) / GeomDist(start, end));
            return ret;
        }
        ...
        */
        ...
        private void ReadAllMissions()
        {
            ...
            for (int i = 0; i < mission.Count; i++)
            {
                ...
                else if (mission[i].ID == 4)
                {
                    //UniqueNavObj
                    VG.Mission.UniqueNavigationObjective UNO = (VG.Mission.UniqueNavigationObjective)mission[i];
                    UniqueNavPoints = new NavigationPointInfo[UNO.NavPoints.Count];
                    for (int j = 0; j < UNO.NavPoints.Count; j++)
                    {
                        UniqueNavPoints[j].Location = UNO.NavPoints[j].Location;
                        UniqueNavPoints[j].StartTurn = UNO.NavPoints[j].StartTurn;
                        UniqueNavPoints[j].EndTurn = UNO.NavPoints[j].EndTurn;
                        UniqueNavPoints[j].Navigators = 0;
                        UniqueNavPoints[j].Complete = false;
                        UniqueNavPoints[j].BotType = UNO.NanoBotType;
                        UniqueNavPoints[j].Stock = UNO.NavPoints[j].Stock;
                        if (UniqueNavPoints[j].Stock > 0)
                            UniqueNavPoints[0].BotType = NanoBotType.NanoCollector;
                    }
                }
            }
            if (count != 0)
        ...
        void UpdateData()
        {
            ...
            if (NavigationPoints != null)
            {
                for (i = 0; i < NavigationPoints.Length; i++)
                {
                    ...
                    //UNTRUSTED!!!
                    if (CurrentTurn > NavigationPoints[i].EndTurn)
                       NavigationPoints[i].Complete = true;
                    //UNTRUSTED!!!
                }
            }
            ...
            if (OtherNanoBotsInfo != null)
            {
                foreach (NanoBotInfo bot in OtherNanoBotsInfo)
                {
                    ...
                    //Если нашли вражьего AI и он идёт к той же точке, что и мы
                    else if ((bot.NanoBotType == NanoBotType.NanoAI) && (bot.PointInfo == this.NextHP))
                    {
                        int zzz1 = 0;
                        Pathfinder.FindPath(bot.Location, this.NextHP, ref zzz1);
                        int zzz2 = 0;
                        Pathfinder.FindPath(this.AI.Location, this.NextHP, ref zzz2);
                        //Причём ему заметно ближе
                        if (zzz1 + 10 < zzz2 && zzz1 > 0 && zzz2 > 0)
                        {
                            //То увы, но это точку мы сдаём
                            for (i = 0; i < HoshimiPoints.Length; i++)
                            {
                                if (bot.PointInfo == HoshimiPoints[i].Location)
                                {
                                    HoshimiPoints[i].Needle = 2;
                                }
                            }
        ...
        */
        ...
        private void AnalizeMap(ref int HPnum, ref int APnum)
        {
            ...
            //Проверка, есть ли хоть одна подходящая точка
            bool MI = true;
        ...
        //Вход - текущий HP-цель
        //Выход неявный - кладёт в список MyHPs путь в виде последовательности номеров HP`ов
        private void BuildPath(int CurHPnum)
        {
            int[] used = new int[HoshimiPoints.Length];
            int sum;    //длина пути
            int last;   //номер последней точки пути
            int tmp;
            int tmpNum;
            int tmpDist;

            //Сбрасываем старый путь, если он был
            MyHPs.Clear();

            //Помечаем все ещё не заюзанные точки
            for (int j = 0; j < used.Length; j++)
            {
                if (HoshimiPoints[j].Needle == 0)
                    used[j] = 0;
                else
                    used[j] = 1;
            }
            //Вписываем первую точку
            MyHPs.Add(CurHPnum);
            used[CurHPnum] = 1;
            sum = 0;
            last = CurHPnum;
            ...
            while (sum < 1500)
            {
                //Ищем HP, ближайшую к последней HP пути
                tmp = 10000;
                tmpNum = -1;
                for (int j = 0; j < HoshimiPoints.Length; j++)
                {
                    if ((HPtoHPdistances[last, j] < tmp) && (used[j] == 0) && (HoshimiPoints[j].Needle == 0))
                    {
                        tmp = HPtoHPdistances[last, j];
                        tmpNum = j;
                    }
                ...
                //Если точки исчерпались, то стоп
                if (tmpNum == -1)
                    break;
                ...
                if (sum < 1500)
                {
                    used[tmpNum] = 1;
                    MyHPs.Add(tmpNum);
                    last = OptimizePath();  //небольшая оптимизация пути
                }
            }
        }
        ...
        //Вход - текущий HP-цель
        //Выход неявный - кладёт в список MyHPs путь в виде последовательности номеров HP`ов
        private void ReBuildPath(int CurHPnum)
        {
            int[] used = new int[HoshimiPoints.Length];
            int sum;    //длина пути
            int last;   //номер последней точки пути
            ...
            MyHPs.Add(CurHPnum);
            used[CurHPnum] = 1;
            HoshimiPoints[CurHPnum].InPast = 0;
            sum = 0;
            last = CurHPnum;

            //Пока ходы есть
            while (CurrentTurn + sum < 1500)
            {
                //Ищем HP, ближайшую к последней HP пути
                tmp = 10000;
                tmpNum = -1;
        ...
        private bool UNOpossible(int HP)
        {
            int tmp=0;
            int sum=0;

            ePathfinder.FindPath(HoshimiPoints[HP].Location, UniqueNavPoints[0].Location, ref sum);
            if (UniqueNavPoints[0].BotType == NanoBotType.NanoCollector)
                sum *= 2;

            for(int i = 1; i < UniqueNavPoints.Length;i++)
            {
                if (sum + 100 > UniqueNavPoints[i-1].EndTurn)
                    return false;
                ePathfinder.FindPath(UniqueNavPoints[i-1].Location, UniqueNavPoints[i].Location, ref tmp);
                if (UniqueNavPoints[0].BotType == NanoBotType.NanoCollector)
                    tmp *= 2;
                sum += tmp;
            }
            if (sum + 100 < UniqueNavPoints[UniqueNavPoints.Length-1].EndTurn)
            {
                return true;
            }
            else
        ...
        private void BuildUNOPath(int HP)
        {
            NavigationPointInfo[] Path = new NavigationPointInfo[UniqueNavPoints.Length];
            int turn;
            int dist;
            int last = -1;
            Point tmp = new Point();

            tmp = HoshimiPoints[HP].Location;
            ...
            for (int i = 0; i < Path.Length; i++)
            {
                ...
                {    
                    if ((UniqueNavPoints[j].Location.X != -1 && UniqueNavPoints[j].EndTurn < turn)
                        || (UniqueNavPoints[j].Location.X != -1 && UniqueNavPoints[j].EndTurn == turn
                        && EstimateMovementTime(tmp, UniqueNavPoints[j].Location) <= dist))
                    {
                        turn = UniqueNavPoints[j].EndTurn;
                        dist = EstimateMovementTime(tmp, UniqueNavPoints[j].Location);
                        last = j;
                    }
                }
                Path[i] = UniqueNavPoints[last];
                tmp = Path[i].Location;
                UniqueNavPoints[last].Location.X = -1;
            }
            for (int i = 0; i < Path.Length; i++)
            {
                UniqueNavPoints[i] = Path[i];
            }
        }
        ...
        void MyAI_WhatToDoNextEvent()
		{
            ...
            //VG.Map.AreaEnum fafa = this.Tissue[88, 85].AreaType;
			...
			foreach (NanoBot bot in this.NanoBots)
			{
                ...
                if (bot is CyclicNavigator)
                {
                    nbCycNav++;
                    ((CyclicNavigator)bot).Action(this);
                }
                if (bot is CyclicDoctor)
                {
                    nbCycDoc++;
                    ((CyclicDoctor)bot).Action(this);
                }
                if (bot is Doctor)
                {
                    nbDoctors++;
                ...
                if (bot is ForwardBodyguard)
                {
                    if (((ForwardBodyguard)bot).registered == false)
                    {
                        RegisterFBG((ForwardBodyguard)bot);
                    }
                    nbFbg++;
                    if (((ForwardBodyguard)bot).Alarm(this))
                    {
                        STOPPED = true;
                    }
                }
            ...
            if ((nbCollector < NBCOLLECTORTOBUILD) || (nbContainer < NBCONTAINERTOBUILD) 
                ...
                || (nbSpy < NBSPY) || BuildConvoy || nbCycNav < NBCYCLICNAV || nbCycDoc < NBCYCLICDOC
                || STOPPED)
            {
                //то останавливаемся.
                AI.StopMoving();
			...
			if (this.AI.State == NanoBotState.WaitingOrders)
			 {
                ...
                if (this.NanoBots.Count < Utils.NbrMaxBots)
                {
                    ...
                    //CyclicNavigator
                    if (nbCycNav < NBCYCLICNAV)
                    {
                        AI.Build(typeof(CyclicNavigator));
                        return;
                    }
                    if (nbCycDoc < NBCYCLICDOC)
                    {
                        AI.Build(typeof(CyclicDoctor));
                        return;
                    }
                    //NanoDoctor
                    ...
                    if (nbFbg < NBFBG)
                    {
                        AI.Build(typeof(ForwardBodyguard));
                        return;
                    }
                    //Observer
		...
		void MyAI_ChooseInjectionPointEvent()
		{
            ...
            */
            ...
            FBGs = new ForwardBodyguard[NBFBG];
            for (int i = 0; i < FBGs.Length; i++)
            {
                FBGs[i] = null;
            }
            ...
            if (UniqueNavPoints != null)
            {
                if (UniqueNavPoints[0].BotType == NanoBotType.NanoExplorer || UniqueNavPoints[0].BotType == NanoBotType.Unknown)
                {
                    NBCYCLICNAV = 2;
                    NBCYCLICDOC = 0;
                }
                else if (UniqueNavPoints[0].BotType == NanoBotType.NanoCollector)
                {
                    NBCYCLICNAV = 0;
                    NBCYCLICDOC = 2;
                }
            ...
            int NotAllocatedBots = Utils.NbrMaxBots - 2 - NBBODYGUARSTOBUILD - NBFBG - NBATACKERTOBUILD - NBPROTECTORTOBUILD - MyHPs.Count - NBCYCLICNAV - NBCYCLICDOC - 2;
