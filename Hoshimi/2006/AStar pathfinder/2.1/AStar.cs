...
namespace PlayerSampleMission1CSharp
{
    ...
    class AStar
    {
        //Version 2.1
        //Changes from 2.0:
        //1) Differents caches for AStar and eAStar classes
        //2) Points in bogs are now computed but their 'G' values very large.
        #region Members
        ...
        {
            #region History checking
            if (explorer)
            {
                for (int i = 0; i < eHistory.Count; i++)
                {
                    if (start == eHistory[i].start && end == eHistory[i].end)
                    {
                        return eHistory[i].path;
                    }
                }
            }
            else
            ...
            if (can)
            {
                ...
                if (explorer)
                    eHistory.Add(new FPath(start, end, ret_p));
                else
        ...
        private void nbPoint(int x, int y, Point end)
        {
            if (x < 0 || x >= X)
                return;
            if (y < 0 || y >= Y)
